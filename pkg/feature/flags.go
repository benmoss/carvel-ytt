...
func (f *Flagset) Enable(name string) *Flagset {
	if f.frozen {
		panic(fmt.Sprintf("Modifying feature flags after they have been frozen can result in strange behavior and is disallowed."))
	}
	f.ensureExists(name)
	f.flags[name] = true
	return f
...
// Freeze marks this Flagset as both ready-for-use and unmodifiable.
func (f *Flagset) Freeze() *Flagset {
	f.frozen = true
	return f
}
...
func (f *Flagset) IsEnabled(name string) bool {
	if !f.frozen {
		panic(fmt.Sprintf("Feature flags must be frozen before first use."))
	}
	f.ensureExists(name)
	return f.flags[name]
}
